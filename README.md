...
当前进度：**Milestone 6 — SSE Real-time Analysis Progress（已完成并验收）**。
...
前端提供视频列表、普通 multipart 上传和元数据详情页面。详情页发起分析后优先使用 `EventSource` 接收排队、提取音频、转录、总结、保存、完成或失败事件；SSE 连接失败时退化为每秒一次、最多 180 次的 GET 查询。终态会关闭连接并自动刷新 Overview、Chapters、Key Points 与 transcript segments；页面刷新时可恢复仍在运行的 taskId。当前尚无视频播放器，因此时间戳仅展示、不提供跳转。单文件上限默认是 500 MB，可通过 `VIDEO_MAX_FILE_SIZE` 与 `VIDEO_MAX_REQUEST_SIZE` 调整。
...
这些测试默认跳过，避免普通单元测试强依赖本机 Docker 或 FFmpeg。M2 基础设施测试检查 MinIO/MySQL 上传链路；M3 基础设施测试回归异步框架；FFmpeg 组件测试覆盖成功、非零退出和超时；M4 基础设施测试覆盖媒体转录链路；M5 基础设施测试覆盖结构化总结；M6 基础设施测试实际捕获完整 SSE 阶段序列，验证终态关闭、四类结果 API 以及 Redis 丢失后的 MySQL 初始状态回退。测试结束会清理自己的临时数据。
...
backend/             Spring Boot API
  ...
  src/main/java/com/videoagent/analysis/ 异步任务、MQ 与实时进度
    controller/event/service              SSE endpoint、订阅生命周期与广播
...
完整清单见 `.env.example`。后端支持通过环境变量覆盖 MySQL、Redis、MinIO、RocketMQ、FFmpeg、分析版本、`VIDEO_ANALYSIS_SSE_TIMEOUT` 以及 LLM Provider 配置。Mock ASR 与 Mock Summary 均不需要第三方 API Key；真实 LLM Key 只从环境变量读取，不写入配置文件或日志。
...
### SSE、Redis 与 MySQL 的职责

进度更新先写入现有 Redis 快照，再向当前应用进程中的 `SseEmitter` 订阅者广播；MySQL 仍是最终事实源。订阅建立时通过查询服务读取初始状态，Redis 缺失时自动回退 MySQL，并在注册后再次读取以缩小竞态窗口。浏览器断开、发送异常、超时和任务终态都会清理 emitter；这些观察通道异常不会传播到 Consumer。按 M6 范围不使用 Redis Pub/Sub，因此多实例部署需要粘性路由或后续独立设计跨实例广播。
...
- 当前前端优先使用 SSE，连接失败时有限退化为 GET；不包含 WebSocket、Redis Pub/Sub 或复杂自动重连框架。
